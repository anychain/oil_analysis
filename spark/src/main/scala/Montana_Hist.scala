...
object Montana_Hist {
  def main(args: Array[String]) {
    ...
    val lease_prod = sqlContext.read.format("com.databricks.spark.csv").options(opts).load(hdfs + "/oil/Montana/histLeaseProd.tab")
    lease_prod.registerTempTable("lease_prod")

    val prod_well = sqlContext.read.format("com.databricks.spark.csv").options(opts).load(hdfs + "/oil/Montana/histprodwell.tab")
    prod_well.registerTempTable("prod_well")

    val well_data = sqlContext.read.format("com.databricks.spark.csv").options(opts).load(hdfs + "/oil/Montana/histWellData.tab")
    well_data.registerTempTable("well_data")

    val opts = Map("header" -> "true", "delimiter" -> ",")

    val df = sqlContext.read.format("com.databricks.spark.csv").options(opts).load(hdfs + "/oil/Montana/Search_Wells.csv")
    ...
    val result = sqlContext.sql("select p.rpt_date, p.API_WELLNO, p.ST_FMTN_CD, p.Name_, p.Lease_Unit, p.OPNO, "
      ...
      + "l.Dt_Receive, l.Del_Rpt, l.StartIvn_OilCd, l.Oil_Prod, l.Gas_Prod, l.Wtr_Prod, "
      ...
      + "l.No_SIWells, "
      + "w.Well_Nm, w.Well_Typ, w.Type, w.Wl_Status, w.Wh_Sec, w.Wh_Twpn, w.Wh_Twpd, w.Wh_RngN, w.Wh_RngD, "
      ...
      + "s.API_NO, s.County, s.Field, s.Dt_Spud, s.Wh_TR, s.Elev_Gr, s.Elev_DF, s.PB_MD, "
